...
const HomePage = () => {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold text-primary mb-4">Ürünler</h1>
        ...
        {products.map((product) => (
          <div key={product.id} className="bg-white p-4 rounded-lg  hover:shadow-lg transition-shadow duration-300">
            <Link to={`/product/${product.id}`}>
              ...
              <p className="text-gray-600">Stok: {product.stockCount}</p>
              <p className="text-gray-600">Yorum Sayısı: {product.feedbacks.length}</p>
            ...
            <div className="mt-4 flex justify-between items-center">
              {isInCart(product.id) ? (
                <span className="text-green-500 font-bold">Ürün zaten sepetinizde</span>
              ) : (
                <button onClick={() => addToCart(product.id, 1)} className="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600">Sepete Ekle</button>
              )}
